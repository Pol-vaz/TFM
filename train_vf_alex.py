...
if __name__ == "__main__":
    ...
    #         plt.yticks(np.arange(0, 0.5, step=0.01)) 
    #         plt.legend()
    #         plt.xlabel('Epochs')  
    #         plt.ylabel('Mse')
    #         plt.show()
    #         fig.savefig(
    #             "snapshots/vf_mse_{}_{}.png".format("diff" if residual_forward else "nodiff", experiment_label),
    #             bbox_inches='tight')
    ...
    print(steps)
    print(lista_steps)
    df = pd.DataFrame((zip(lista_steps, lista_exitos)), columns = ['Steps', 'Exitos'])
    print(df[0:10])
