...
def simulador_exitos(perfection = False, n = 100):
    print('\nSimulador\n')
    n = n
    ...
    print(df1[0:10])
    return exitos, intentos, steps, fracasos

def generar_imgsimulador( n = 100):
...
def aplicar_accion_simulador(action, n = 100):
    ...
    # Muestro figura
    target, robot = sim.objects_pos
    print(f'\nAngulo original -> {target} {robot}\n')
    #saco mejor angulo
    import math
    angle_2 = math.atan2(robot[1]- target[1], robot[0] - target[0])
    angle_deg_2 = (math.degrees(angle_2) + 90) * -1
    print(angle_deg_2)
    print(angle_deg_2 /360)
    #muestro imagen
    print('\nImagen Original\n')
    sim.show_image(sim.iter - 1) 
    #Aplico accion
    sim.apply_action(angle_deg_2)
    # Muestro figura nueva
    print('\nImagen Nueva\n')
    target_new, robot_new = sim.objects_pos
    print(f'\nAngulo Nuevo -> {target_new} {robot_new}\n')
    sim.show_image(sim.iter - 1)  

    # sim.apply_action(angle_normalized * 180)
    # # Muestro figura
    # sim.show_image(sim.iter - 1) 
    ...
    if generador:
        ...
        try:
            for i in range(1, n):
                ...
                print(type(actual))
                sim.show_image(sim.iter - 1)
                robot, target = sim.objects_pos
                print(robot, target)
                import math
                angle = math.atan2(target[1]- robot[1], target[0] - robot[0])
                print(angle)
        ...
        best_action_normalized =scaler.fit_transform(best_actions_exact)
        angles = list(zip(best_action_list, exact_angles, best_action_normalized))
        df = pd.DataFrame(angles, columns=['Best Action', 'Real Angle', 'Real Angle Normalized'])
        print(df.head())
        df.to_csv('generador_policy/Best_actions/'+ruta+'/Best_action_batch.csv', index=False)

        return input_images, best_actions, best_action_normalized
...
if __name__ == "__main__":
    ...
    carga = False
    if carga:
        X_train =load_images("C:/Users/pablo/Desktop/CodigoTFM/disentangled_experiments-main/disentangled_experiments-main/generador_policy/Train")
        print(X_train.shape)
        obj_size = (64,64)
        for i in X_train:
            print(i.shape)
    ...
    df_train = pd.read_csv("C:/Users/pablo/Desktop/CodigoTFM/disentangled_experiments-main/disentangled_experiments-main/generador_policy/Best_actions/Train/Best_action_batch.csv")
    df_test = pd.read_csv("C:/Users/pablo/Desktop/CodigoTFM/disentangled_experiments-main/disentangled_experiments-main/generador_policy/Best_actions/Test/Best_action_batch.csv")
    ...
    # random_actions = [-1, 1, 0, -0.5, 0.5]   
    # for act in range(10):   
    #     aplicar_accion_simulador(action = 50)
    #cargo modelo policy maximo
    policy = KerasNN(input_neurons= latent_space_manual, verbose = 1)
    model = policy.get_model()
    print(model.summary())
    path_model= "C:/Users/pablo/Desktop/Policy_model_{}.h5".format(experiment_label)
    model = policy.load_model(path_model)

    exitos, intentos_perfe, steps, fracasos = simulador_exitos(perfection = True, n = 100)
    exitos, intentos, steps, fracasos = simulador_exitos(perfection = False, n = 100)
    print(exitos, intentos, steps, fracasos)
    print('\n\n')
    print(f'Porcentaje de Exito -> {(exitos/intentos)*100}%')
    print(f'Porcentaje de Exito Teorico -> {(exitos/intentos_perfe)*100}%')
    ...
    if test:
        ...
        predictions = policy.predict(input_images_testeo)
        print(f'Mejores acciones Predichas -> \n{predictions[0:10]*360}\n')
        print(f'Mejores acciones reales -> \n{best_actions_exact_testeo[0:10]*360}\n')
        
        # exitos, intentos, steps = simulador_exitos(perfection = False)
        # print(exitos, intentos, steps)
        # print('\n\n')
        # print(f'Porcentaje de Exito -> {(exitos/intentos)*100}%')

        #Esperado VS Real
        input('\nEsperado VS Real\n\nPulsa una tecla para continuar...\n')
        for i in range(len(input_images_train)):
            policy_prediction = policy.predict(input_images_train)

            fig = plt.figure()
            # sort freq
            best_actions_test = np.sort(best_actions_train, axis = 0)
