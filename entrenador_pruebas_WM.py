...
from models import build_conv_only_ae, make_forward_model_5_layers, make_forward_model_3_layers, make_forward_model_10_layers
...
import os
os.chdir("C:/Users/pablo/Desktop/CodigoTFM/disentangled_experiments-main/disentangled_experiments-main")
...
def create_fm_3(latent_size, residual_forward, activation_fun = 'relu'):
    forward_model = make_forward_model_3_layers([latent_size], latent_size, learn_only_difference=residual_forward, activation_fun= activation_fun)
    #forward_model = make_forward_model_3_layers([latent_size], latent_size, learn_only_difference=residual_forward)
    #forward_model = make_forward_model_10_layers([latent_size], latent_size, learn_only_difference=residual_forward)
    forward_model.compile(loss='mse', optimizer=prepare_optimizer_object('adam', 0.001), metrics=['accuracy','mse'])
    forward_model.summary()
    return forward_model

def create_fm_5(latent_size, residual_forward, activation_fun = 'relu'):
    #forward_model = make_forward_model([latent_size], latent_size, learn_only_difference=residual_forward)
    forward_model = make_forward_model_5_layers([latent_size], latent_size, learn_only_difference=residual_forward, activation_fun= activation_fun)
    #forward_model = make_forward_model_10_layers([latent_size], latent_size, learn_only_difference=residual_forward)
    forward_model.compile(loss='mse', optimizer=prepare_optimizer_object('adam', 0.001), metrics=['accuracy','mse'])
    forward_model.summary()
    return forward_model

def create_fm_10(latent_size, residual_forward, activation_fun = 'relu'):
    #forward_model = make_forward_model([latent_size], latent_size, learn_only_difference=residual_forward)
    #forward_model = make_forward_model_3_layers([latent_size], latent_size, learn_only_difference=residual_forward)
    forward_model = make_forward_model_10_layers([latent_size], latent_size, learn_only_difference=residual_forward, activation_fun= activation_fun)
    forward_model.compile(loss='mse', optimizer=prepare_optimizer_object('adam', 0.001), metrics=['accuracy','mse'])
    forward_model.summary()
    return forward_model
...
def evaluate_decoded_images():
    ...
    np.savetxt('snapshots/WM/'+str(latent_space_manual)+'/forward_{}_{}_{}.eval'.format("diff" if residual_forward else "nodiff", experiment_label, arqui),
...
def plot_results():
    ...
    for i in range(n):
        ...
        if i == 0:
            ...
            ax.get_yaxis().set_visible(False)
    fig.savefig('snapshots/WM/'+str(latent_space_manual)+'/forward_{}_{}_{}_bs100.pdf'.format("diff" if residual_forward else "nodiff", experiment_label, arqui),
                bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    #voy a meterlo todo dentro de un loop de train
    layers_list = [create_fm_3, create_fm_5, create_fm_10]
    layer_name = ['3_Layers', '5_Layers', '10_Layers']
    activation_list = ['relu', 'sigmoid']
    loss = []
    val_loss = []
    latent_space_manual = 9
    
    for layer, layer_str in zip(layers_list, layer_name):
        for fun in activation_list:
            # Creo forward model
            forward_model = layer(latent_size, residual_forward, activation_fun = fun )
            arqui = f"{layer_str}-{fun})"
            ...
            if do_train:
                ...
                # plt.show()
                fig.savefig('snapshots/forward_loss_{}_{}_{}_arqRich6.png'.format("diff" if residual_forward else "nodiff", experiment_label, arqui),
                    bbox_inches='tight')
                forward_model.save(
                    'trained_models/WM/'+str(latent_space_manual)+'/forward_model_{}_{}_{}_arqRich6.h5'.format("diff" if residual_forward else "nodiff", experiment_label, arqui))
                loss_list = list(zip(history, val_history))
                df = pd.DataFrame(loss_list, columns=['Loss','Val Loss'])
                print(df.head())
                    
                df.to_csv('trained_models/WM/'+str(latent_space_manual)+'/loss_'+arqui+'.csv', index=False)
            ...
            if do_test:
                ...
                batch_size = latent_space_manual
